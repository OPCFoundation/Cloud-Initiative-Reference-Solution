...
|------------|------------------------------|------------------------|----------------------------------------------|
| **Spoofing** (identity) | &bull; A rogue client impersonates the Publisher or **UA Cloud Commander/Action** to the broker<br>&bull; An attacker impersonates a web UI user (Translator, Publisher, Grafana, UA Cloud Action, or Portainer)<br>&bull; A fake OPC UA server feeds the Publisher<br>&bull; A forged `ua-action-request` triggers an OPC UA method<br>&bull; An unauthenticated caller hits the **OPC UA Web API**<br>&bull; Anything on the pod network impersonates a Modbus master<br>&bull; **theft of the Publisher's CA key (`/publisher/pki/issuer/private`) lets an attacker mint a trusted certificate for any component**<br>&bull; **anyone who can reach the UA Cloud Library UI (`:8083`) can self-register a working account and act as a legitimate user.** | &bull; MQTT broker requires username/password (`allow_anonymous false`)<br>&bull; Most web UIs (`:8080/:8081/:8082/:3000/:9443`) require login<br>&bull; The **UA Cloud Action web UI and OPC UA Web API mandate HTTP Basic authentication on every request (no anonymous access)**<br>&bull; OPC UA supports certificate exchange between Publisher/Commander and server<br>&bull; The Cloud Library requires an account to upload, and its API is authenticated with `ServiceUsername`/`ServicePassword` | &bull; Single shared credential set across all components (including Grafana/Portainer admin and the Web API)<br>&bull; Basic-auth credentials are only as safe as the transport (send over TLS in production)<br>&bull; No per-service identities or mutual TLS (mTLS)<br>&bull; Broker does not authenticate clients by certificate<br>&bull; Any client that can publish to `commands` can drive Commander<br>&bull; **the GDS issuer key is a 12-year self-signed CA stored in a PKCS#12 with an empty password on a `hostPath` volume** (see hardening item 9)<br>&bull; **the Cloud Library has email verification disabled (`EmailSenderAPIKey` unset), so self-registration is open and accounts are not tied to a provable identity**<br>&bull; **MQTT Explorer (`:4000`) has no authentication of its own, so anyone who can reach it can publish to any topic — including `commands`**<br>&bull; **Modbus TCP has no authentication whatsoever by protocol design** — the simulator (and any real Modbus device) trusts every caller |
| **Tampering** (integrity) | &bull; Modification of telemetry in transit<br>&bull; Tampering with `hostPath` config/cert files on the node<br>&bull; Editing the ConfigMaps<br>&bull; **altering the imported `opcua_model` data** or the model importer script<br>&bull; A malicious command writing/actuating an OPC UA node via Commander<br>&bull; Writing Modbus coils/registers on the simulated device<br>&bull; **forging or altering a stored Digital Product Passport** so a product appears to have a lower carbon footprint than it does, or uploading a malicious nodeset that is then trusted as the definition of what a machine reports | &bull; MQTT is carried over TLS (8883)<br>&bull; Config is delivered via Kubernetes ConfigMaps/Secrets<br>&bull; Commander/Action send spec-compliant OPC UA PubSub Action envelopes<br>&bull; The seeded Thing Description and settings are delivered read-only from ConfigMaps<br>&bull; Cloud Library uploads require an authenticated account | &bull; Telegraf and UA Cloud Action use TLS verification skip (`insecure_skip_verify` / `MQTT_TLS_INSECURE=true`), so a man-in-the-middle with any cert is accepted<br>&bull; `hostPath` volumes (`/influxdb2`, `/cloudlib-postgres`, `/translator/*`, `/publisher/*`, `/commander/*`, `/productionline/*`, `/mosquitto`, `/portainer`, `/grafana`) are writable by anyone with node access<br>&bull; No message signing on payloads<br>&bull; Commander performs Writes/MethodCalls with no per-action authorization<br>&bull; **stored Digital Product Passports are not signed or provenance-checked, and because registration is open any account can upload one, so a passport carries no cryptographic proof of origin**<br>&bull; **PCF and Battery Passport results are published without a signature, so a consumer, recycler or regulator cannot verify they came from this pipeline**<br>&bull; **Modbus traffic is plaintext and unauthenticated**, so anything on the pod network can read or write the simulated device's registers |
| **Repudiation** (auditability) | &bull; An operator changes a device mapping, publish set, Grafana dashboard, or issues a command and denies it<br>&bull; No record of who logged in or who imported a model<br>&bull; **a user uploads a Digital Product Passport to the Cloud Library and denies it**<br>&bull; **a disputed Product Carbon Footprint cannot be traced back to the inputs it was derived from**, which matters when the passport is presented as a regulatory claim | Component logs are written to `hostPath` `logs` directories and pod stdout; Portainer records some cluster events; the Cloud Library records the owning account against each upload. | &bull; No centralized, tamper-evident audit log<br>&bull; Shared credentials make actions unattributable to an individual<br>&bull; Command/action requests and model imports are not attributably logged<br>&bull; **Cloud Library accounts are self-registered with unverified email addresses, so the recorded uploader identity is weak evidence**<br>&bull; **UA Data Processor does not retain the telemetry window or carbon-intensity figure behind each PCF, so a passport's figures are not independently reproducible**<br>&bull; No log shipping or retention policy |
| **Information disclosure** (confidentiality) | &bull; Sniffing telemetry<br>&bull; Reading credentials from the manifest<br>&bull; Exposed dashboards (Grafana, Portainer, UA Cloud Action) on the node IP<br>&bull; Leaking the **UA Cloud Library credentials** used by the import Job<br>&bull; **reading an OPC UA private key — or the Publisher's CA key — off the node (or off the SSD if the device is removed)**<br>&bull; **reading the Cloud Library's PostgreSQL database directly off `/cloudlib-postgres`, which exposes every stored Digital Product Passport and all account password hashes**<br>&bull; **inferring production volumes, energy use and product composition from stored passports.** | &bull; MQTT is encrypted with TLS<br>&bull; `INFLUX_TOKEN` is stored in a Kubernetes `Secret`<br>&bull; Credentials are supplied at apply time (not committed to git)<br>&bull; PostgreSQL is `ClusterIP` only, so it is not reachable from outside the cluster<br>&bull; Cloud Library passwords are stored as ASP.NET Identity hashes, not plaintext | &bull; Credentials (including UA Cloud Library and Grafana/Portainer admin) are injected as plain-text env vars (visible via `kubectl describe`/`exec`)<br>&bull; Kubernetes Secrets are base64, not encrypted at rest by default<br>&bull; Self-signed broker cert offers encryption but no server-identity assurance<br>&bull; **OPC UA private keys, including the GDS issuer (CA) key, are held unprotected in `Directory` stores on `hostPath` volumes** (see hardening item 9)<br>&bull; **the PostgreSQL data directory is an unencrypted `hostPath` and the database password is the shared `IOT_PASSWORD`**<br>&bull; **the Cloud Library is served over plain HTTP, so registration and login credentials cross the network in the clear**<br>&bull; All UIs are exposed on the node IP with no network policy |
| **Denial of service** (availability) | &bull; Flooding the broker or web UIs<br>&bull; Filling the node disk with telemetry or repeated model imports<br>&bull; A crash loop<br>&bull; A runaway feedback loop from UA Cloud Action<br>&bull; Overloading the simulated stations or the Modbus simulator with connections<br>&bull; **filling the disk by uploading large or numerous passports/nodesets to the Cloud Library**<br>&bull; **exhausting InfluxDB with the Data Processor's repeated multi-day queries.** | &bull; Liveness/readiness probes restart unhealthy pods<br>&bull; Single-replica deployments recover automatically<br>&bull; The importer is a short-lived Job with `ttlSecondsAfterFinished`<br>&bull; **UA Cloud Action has a built-in rate limiter that bounds how often it actuates**<br>&bull; The Modbus simulator declares CPU/memory `requests`/`limits`<br>&bull; The Data Processor polls on a fixed interval rather than continuously | &bull; No rate limiting, quotas, or `resources` requests/limits on most pods<br>&bull; Unbounded InfluxDB growth on the local SSD (now including `opcua_model` points)<br>&bull; **no upload size limit or per-account quota on the Cloud Library, and its PostgreSQL volume has no size cap — filling `/cloudlib-postgres` fills the same disk InfluxDB and the broker rely on**<br>&bull; A single node is a single point of failure<br>&bull; The broker persists to `hostPath` (`/mosquitto`), reducing message loss on restart though the single node remains a SPOF<br>&bull; UA Cloud Action's rate limit still needs tuning for your environment |
| **Elevation of privilege** (authorization) | &bull; Container escape to the node<br>&bull; A compromised pod reading another component's data via shared host paths<br>&bull; Using the InfluxDB admin token for full DB control<br>&bull; **abusing Portainer's `cluster-admin` ServiceAccount to take over the whole cluster**<br>&bull; Using Commander/Action to reach and control OT devices<br>&bull; **a self-registered Cloud Library user escalating to administrative rights over the model store**<br>&bull; **a compromised UA Data Processor reusing the admin `INFLUX_TOKEN` it is given.** | &bull; Distinct container images per component<br>&bull; `nodeSelector` pins workloads to Linux<br>&bull; The importer Job uses `restartPolicy: Never`<br>&bull; The Cloud Library separates ordinary user accounts from the `ServiceUsername` API account | &bull; Containers run with default (often root) user and no `securityContext`<br>&bull; No `NetworkPolicy` isolation between pods<br>&bull; The InfluxDB token is an all-powerful admin token<br>&bull; **UA Data Processor only ever reads, but is handed the same admin token rather than a read-only one**<br>&bull; **Portainer is bound to `cluster-admin`, so compromising it compromises the cluster**<br>&bull; **the Cloud Library's API account shares the single `IOT_PASSWORD` used everywhere else, so one leaked credential grants model-store write access**<br>&bull; Commander bridges IT→OT with method-call/write capability and no fine-grained authorization<br>&bull; No RBAC scoping for the workloads |
